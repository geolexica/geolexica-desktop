...
import { NonIdealState, Checkbox, Button, Callout, TextArea, Toaster, Position, Popover } from '@blueprintjs/core';
...
const modifiedFileListing: { [K in keyof Storage]: React.FC<ModifiedFileOverviewProps<Storage[K] & any>> } = {
  ...
  concepts: ({ items, selectedItems, onSelect }) => <>
    <PaneHeader align="left">{items.length} modified {items.length !== 1 ? "concepts" : "concept"}</PaneHeader>

    <div className={styles.itemList}>
      {items.map(item => <ItemCard
        title={item.term}
        onEdit={() => openWindow('concept', { id: item.id })}
